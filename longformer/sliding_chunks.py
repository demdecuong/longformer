...
def pad_window_size(input_ids: torch.Tensor, one_sided_window_size: int, pad_token_id: int):
    w = 2 * one_sided_window_size
    seqlen = input_ids.size(0)
    padding_len = (w - seqlen % w) % w
    input_ids = F.pad(input_ids.permute(1,0), (0,padding_len), value=pad_token_id)
    return input_ids.permute(1,0)