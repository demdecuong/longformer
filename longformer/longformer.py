...
class LongformerSelfAttention(nn.Module):
    def __init__(self, hidden_size = 512,
                ...
                attention_window = [256 , 256 , 256, 256],
                attention_dilation = [1, 1, 1, 1],
                layer_id = 1):
        ...
        self.embed_dim = hidden_size #768
        ...
        self.dropout = attention_probs_dropout_prob # 0.1

        self.layer_id = layer_id
        self.attention_window = attention_window[self.layer_id]   # list of [256,256,... , 256]
        self.attention_dilation = attention_dilation[self.layer_id]
        self.attention_mode = attention_mode
        self.autoregressive = autoregressive # False --> Next token prediction
        assert self.attention_window > 0
        assert self.attention_dilation > 0
        assert self.attention_mode in ['tvm', 'sliding_chunks']  # 'sliding_chunks'
        if self.attention_mode == 'sliding_chunks':
            assert not self.autoregressive  # not supported
            assert self.attention_dilation == 1  # dilation is not supported  # list of ones vector
        #  "bos_token_id": 0,
        #  "eos_token_id": 2,
    ...
    def forward(
        ...
        if remove_from_windowed_attention_mask is not None:
            # This implementation is fast and takes very little memory because num_heads x embed_dim = 1
            # from (bsz x seq_len) to (bsz x seq_len x num_heads x embed_dim)
