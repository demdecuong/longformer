''' Transformer and Longformer Layer '''
...
class LongformerLayer(nn.Module):
    ...
    def __init__(self, embed_dim, ff_embed_dim, num_heads,layer_id, dropout, with_external=False, weights_dropout = True):
        super(LongformerLayer, self).__init__()
        self.self_attn = LongformerSelfAttention(embed_dim,num_heads,dropout,layer_id=layer_id)
        ...
        if self.with_external:
            self.external_attn = MultiheadAttention(embed_dim, num_heads, dropout, weights_dropout,True)
    ...
    def forward(self, x, kv = None, attention_mask=None,
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, need_weights=False):
        ...
        if self.with_external:
            ...
            attn_weights = cal_self_attention(q,k.transpose(1,2))
...
def cal_self_attention(q,k,w = 5):
    '''
    Calculate self-attention
    Input:
        q:  torch.Tensor (bs*n_head,seq len , dim // n_head)
        k:  torch.Tensor (bs*n_head, dim // n_head, seq len)
    Output:
    Return torch.Tensor (q.shape[0],k.shape[1])
    '''
    bs,seq_len,d_model = q.size()
    out = torch.zeros((bs,seq_len,k.shape[-1]))
    for i in range(seq_len):
        if i >= w and i + w <= seq_len:
            tmp = k[:,:, i-w:i+w+1]
            tmp = q[:,i].unsqueeze(-1) * tmp
            out[:,i] = tmp.sum()
        elif seq_len - i < w:
            tmp = k[:,:, i-w:seq_len]
            tmp = q[:,i].unsqueeze(-1) * tmp
