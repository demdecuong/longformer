...
class BatchData:
    def __init__(self, flist, modules, consts, options):
        ...
        self.y_inp = np.zeros((consts["len_y"], self.batch_size), dtype = np.int64)
        ...
        # self.attention_mask = torch.ones(self.x.shape, dtype=torch.long) # initialize to local attention
        ...
        # self.mask_x = torch.ones(self.x.shape, dtype=torch.long, device='cuda') # initialize to local attention
        # self.pad_token_idx = 0

        # self.x = torch.LongTensor(self.x)
        # self.x, self.mask_x = pad_to_window_size(
        #                     self.x.permute(1,0), self.mask_x.permute(1,0), 256, self.pad_token_idx)
        # self.x = self.x.permute(1,0)

def get_data(xy_list, modules, consts, options):
    # print(modules["i2w"][1],modules["i2w"][2],modules["i2w"][3]) <unk> <bos> <eos>
